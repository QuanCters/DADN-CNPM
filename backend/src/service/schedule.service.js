...
class ScheduleService {
  static createSchedule = async ({ device_id, action_time, action, value }) => {
    if (!device_id || !action_time || !action || (action == "on" && !value)) {
      throw new Error("Missing required fields");
    }
    if(!value) value = 0;
    const device = await getDeviceById(device_id);
    if (!device) throw new NotFoundError("Device not found");
    const history = await getSchedule({device_id, action_time: new Date(action_time)});
    if (history){
      throw new ConflictRequestError("Schedule\'device already exists for this time");
    }
    const schedule = await createSchedule({ device_id, action_time: new Date(action_time), action, value });
  ...
  static createScheduleFull = async ({ device_id, time_on, time_off, value }) => {
    const device = await getDeviceById(device_id);
    if (!device) throw new NotFoundError("Device not found");
    const historyOn = await getSchedule({device_id, action_time: new Date(time_on)});
    if (historyOn){
      throw new ConflictRequestError("Schedule\'device already exists for this time");
    }
    const historyOff = await getSchedule({device_id, action_time: new Date(time_off)});
    if (historyOff){
      throw new ConflictRequestError("Schedule\'device already exists for this time");
    }
    ...
    const schedule_on = await createSchedule({ device_id, action_time: new Date(time_on), action: "on" , value });
    const schedule_off = await createSchedule({ device_id, action_time: new Date(time_off), action: "off", value: 0 });
    if(!schedule_on || !schedule_off) throw new ConflictRequestError("Failed to create schedule");
  ...
  static updateSchedule = async ({ device_id, action_time, action, action_time_old, value }) => {
    const device = await getDeviceById(device_id);
    if (!device) throw new NotFoundError("Device not found"); 
    ...
    const updatedSchedule = await updateDeviceSchedule({device_id, action_time: new Date(action_time), action, action_time_old: new Date(action_time_old), value});
    if (!updatedSchedule) throw new ConflictRequestError("Failed to update schedule");
    return ({
